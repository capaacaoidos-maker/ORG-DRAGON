...
function buildQueueButtons(mode, value) {
  ...
  return [
    new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`queue_normal_${mode}_${v}`).setLabel('Gel Normal').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`queue_infinito_${mode}_${v}`).setLabel('Gel Infinito').setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId(`queue_sair_${mode}_${v}`).setLabel('Sair da Fila').setStyle(ButtonStyle.Danger),
    ),
    new ActionRowBuilder().addComponents(
      new ButtonBuilder().setCustomId(`queue_arma_${mode}_${v}`).setLabel('FULL UMP XM8').setStyle(ButtonStyle.Primary),
    ),
  ];
}

// Estado por modo+valor
function getQueueKey(mode, value) { return `${mode}_${parseFloat(value).toFixed(2)}`; }

function getQueueStateByKey(key) {
  if (!queues[key]) queues[key] = { players: [], value: parseFloat(key.split('_')[1]) };
  return queues[key];
}
...
client.on('messageCreate', async (message) => {
  ...
  // !registrar pix <chave> — só o cargo confirmador pode registrar
  if (cmd === 'registrar') {
    if (args[0]?.toLowerCase() !== 'pix') return;
    const temCargo = message.member.roles.cache.has('1495603958780137683');
    const isAdmin  = message.member.permissions.has(PermissionFlagsBits.Administrator);
    if (!temCargo && !isAdmin) return message.reply('❌ Só quem pode confirmar AP pode registrar PIX!');
    const chave = args[1];
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (id.startsWith('adm_confirmar_ap_')) {
      const temCargo = interaction.member.roles.cache.has('1495603958780137683');
      const isAdm   = interaction.member.permissions.has(PermissionFlagsBits.Administrator);
      if (!temCargo && !isAdm) return interaction.reply({ content: '❌ Sem permissão para confirmar AP.', ephemeral: true });
      ...
      // Mostrar PIX dos jogadores pro adm (só ele vê)
      const pixList = filaInfo.players.map(p => {
        const pix = config.pixUsers?.[p.id];
        return `<@${p.id}> — \`${p.nick}\`\nPIX: ${pix ? `\`${pix}\`` : '❌ Não registrado'}`;
      }).join('\n\n');

      // Atualizar mensagem com botão "AP Feito"
      await interaction.update({
        embeds: [new EmbedBuilder().setColor(0xFFAA00)
          .setTitle('⏳ AP EM ANDAMENTO — ORG TIGRE')
          .setDescription(
            `Confirmado por ${interaction.user} — faça o AP no jogo!\n\n` +
            `**💳 PIX dos jogadores:**\n${pixList}\n\n` +
            `> Quando terminar o AP clique em **"AP Feito"**`
          )
          .setFooter({ text: 'ORG TIGRE' }).setTimestamp()],
        components: [new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId(`adm_ap_feito_${filaChId}_${interaction.user.id}`).setLabel('✅ AP Feito').setStyle(ButtonStyle.Success),
          new ButtonBuilder().setCustomId(`adm_recusar_ap_${filaChId}`).setLabel('❌ Cancelar').setStyle(ButtonStyle.Danger),
        )]
      });
      return;
    }

    // AP Feito — agora manda o PIX do adm pros jogadores
    if (id.startsWith('adm_ap_feito_')) {
      const parts = id.replace('adm_ap_feito_', '').split('_');
      ...
      await interaction.update({
        embeds: [new EmbedBuilder().setColor(0x00C896)
          .setTitle('✅ AP FEITO — ORG TIGRE')
          .setDescription(`AP concluído por ${interaction.user}!`)
          .setFooter({ text: 'ORG TIGRE' }).setTimestamp()],
        components: []
      });

      // Agora manda PIX do adm no canal da fila
      try {
        const filaCh = await client.channels.fetch(filaChId);
        await filaCh.send({ embeds: [new EmbedBuilder().setColor(0x00C896)
          .setTitle('✅ AP FEITO! Pague agora!')
          .setDescription(
            `> O AP foi feito! Agora pague o ADM! 💰\n\n` +
            `**📲 PIX do ADM (${interaction.user.username}):**\n` +
            `\`\`\`${admPix || 'ADM não registrou PIX!'}\`\`\``
          )
          .setFooter({ text: 'ORG TIGRE • Free Fire' }).setTimestamp()] });
      } catch {}
      return;
      ...
      if (filaInfo.confirmed.length >= filaInfo.players.length) {
        ...
        await interaction.channel.send({ embeds: [new EmbedBuilder().setColor(0x00C896)
          ...
          .setDescription(
            ...
            `📊 **Taxa ADM:** R$ ${TAXA.toFixed(2).replace('.', ',')}\n\n` +
            `> Aguarde o ADM confirmar o AP. O PIX será enviado após a confirmação!`
          ).setFooter({ text: 'ORG TIGRE • Free Fire' }).setTimestamp()] });
      }
      return;
    }
