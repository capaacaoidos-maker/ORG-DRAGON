...
function buildQueueEmbed(mode, value) {
  ...
  const playerList = state.players.length > 0
    ? state.players.map((p, i) => `\`${i + 1}.\` <@${p.id}> — \`${p.nick || p.name}\` ${p.arma ? `[${p.arma}]` : ''}`).join('\n')
...
function buildQueueButtons(mode) {
  const row1 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId(`queue_fechar_${mode}`).setLabel('🔒 Fechar Fila').setStyle(ButtonStyle.Danger),
  );
  const row2 = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId(`queue_arma_${mode}`)
      .setPlaceholder('🔫 Escolha sua arma...')
      .addOptions([
        { label: 'FULL UMP', value: 'FULL UMP', emoji: '🔫' },
        { label: 'FULL XM8', value: 'FULL XM8', emoji: '🔫' },
        { label: 'FULL M1887', value: 'FULL M1887', emoji: '🔫' },
        { label: 'FULL AK', value: 'FULL AK', emoji: '🔫' },
        { label: 'FULL M4A1', value: 'FULL M4A1', emoji: '🔫' },
        { label: 'LIVRE', value: 'LIVRE', emoji: '⚔️' },
      ])
  );
  return [row1, row2];
}
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
        ...
        if (state.players.length >= cfg.maxPlayers) {
          const fullEmbed = new EmbedBuilder()
            ...
            .setDescription(
              ...
              `**Jogadores:**\n` +
              state.players.map((p, i) => `\`${i + 1}.\` <@${p.id}> — \`${p.nick}\` (Gel ${p.gel}) [${p.arma || 'LIVRE'}]`).join('\n') +
              `\n\n> ⚔️ Boa sorte a todos!`
            )
            .setTimestamp();
    ...
    // Fechar fila
    if (id.startsWith('queue_fechar_')) {
      const mode = id.replace('queue_fechar_', '');
      const isAdmin = interaction.member.permissions.has(PermissionFlagsBits.Administrator);
      const isStaff = config.staffRole && interaction.member.roles.cache.has(config.staffRole);
      if (!isAdmin && !isStaff) return interaction.reply({ content: '❌ Sem permissão para fechar a fila.', ephemeral: true });

      queues[mode] = { players: [], value: getQueueState(mode).value };
      try {
        await interaction.message.edit({
          embeds: [buildQueueEmbed(mode, getQueueState(mode).value)],
          components: buildQueueButtons(mode),
        });
      } catch {}
      return interaction.reply({ content: `🔒 Fila **${mode}** foi fechada e resetada!`, ephemeral: true });
    }
  ...
  // ── ARMA SELECT MENU ───────────────────────────────────────────────────────
  if (interaction.isStringSelectMenu() && interaction.customId.startsWith('queue_arma_')) {
    const mode = interaction.customId.replace('queue_arma_', '');
    const arma = interaction.values[0];
    const state = getQueueState(mode);
    const player = state.players.find(p => p.id === interaction.user.id);

    if (!player) return interaction.reply({ content: '⚠️ Entre na fila primeiro antes de escolher a arma!', ephemeral: true });
    ...
    return interaction.reply({ content: `✅ Arma definida: **${arma}**`, ephemeral: true });
  }
