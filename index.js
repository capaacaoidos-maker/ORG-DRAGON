...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (id.startsWith('adm_confirmar_ap_')) {
      ...
      // Verificar se o confirmador tem PIX registrado
      const admPix = config.pixUsers?.[interaction.user.id];
      if (!admPix) return interaction.reply({ content: '❌ Você precisa registrar seu PIX antes de confirmar!\nUse: `!registrar pix suachave`', ephemeral: true });
      ...
      // Mandar PIX do confirmador no canal da fila
      try {
        const filaCh = await client.channels.fetch(filaChId);
        await filaCh.send({ embeds: [new EmbedBuilder().setColor(0x00C896)
          .setTitle('✅ AP CONFIRMADO!')
          .setDescription(
            `> O AP foi confirmado! 🎉\n\n` +
            `**📲 PIX do Confirmador (${interaction.user.username}):**\n` +
            `\`\`\`${admPix}\`\`\``
          )
          .setFooter({ text: 'ORG TIGRE • Free Fire' }).setTimestamp()] });
      } catch {}
