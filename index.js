...
client.on('messageCreate', async (message) => {
  ...
  // !allfilas <modo> [canalId]
  // Exemplos:
  // !allfilas 1x1          → manda todos os valores do 1x1 no canal atual
  // !allfilas 1x1 123456   → manda todos os valores do 1x1 no canal 123456
  // !allfilas 1x1 100      → manda só o valor 100 do 1x1 no canal atual
  // !allfilas 1x1 100 123456 → manda só o valor 100 do 1x1 no canal 123456
  if (cmd === 'allfilas') {
    if (!isAdmin) return message.reply('❌ Sem permissão.');
    ...
    // Detectar se args[1] é um valor ou id de canal
    let targetValue = null;
    let targetChannelId = null;

    if (args[1]) {
      const num = parseFloat(args[1]);
      if (!isNaN(num) && PRICES.includes(num)) {
        targetValue = num;
        targetChannelId = args[2] || null;
      } else {
    ...
    if (targetChannelId) {
      try { targetChannel = await client.channels.fetch(targetChannelId); }
      catch { return message.reply('❌ Canal não encontrado. Verifique o ID.'); }
    }

    const pricesToSend = targetValue ? [targetValue] : PRICES;

    for (const price of pricesToSend) {
      const state = getQueueState(mode);
      state.value = price;
      await targetChannel.send({ embeds: [buildQueueEmbed(mode, price)], components: buildQueueButtons(mode) });
      await new Promise(r => setTimeout(r, 400));
    }

    message.delete().catch(() => {});
    if (targetChannelId) message.reply(`✅ Filas do **${mode}** enviadas em <#${targetChannelId}>!`).then(m => setTimeout(() => m.delete().catch(() => {}), 3000));
  }
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (id.startsWith('queue_normal_') || id.startsWith('queue_infinito_')) {
      ...
      if (state.players.length >= cfg.maxPlayers) await criarCanalFila(interaction, mode, state);
      return;
    }
    ...
    if (id.startsWith('queue_arma_')) {
      ...
      if (player.arma === 'FULL UMP XM8') return interaction.reply({ content: '⚠️ Você já escolheu!', ephemeral: true });
      player.arma = 'FULL UMP XM8';
      try { await interaction.message.edit({ embeds: [buildQueueEmbed(mode, state.value)], components: buildQueueButtons(mode) }); } catch {}
      await interaction.reply({ content: `✅ Arma definida: **FULL UMP XM8**`, ephemeral: true });
      return;
    }
