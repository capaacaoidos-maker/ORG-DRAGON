...
const PRICES = [0.30, 0.50, 0.75, 1.00, 2.00, 5.00, 10.00, 20.00, 30.00, 50.00, 75.00, 80.00, 100.00, 200.00, 300.00];
...
function buildQueueEmbed(mode, value) {
  ...
  const playerList = state.players.length > 0
    ? state.players.map((p, i) => `\`${i + 1}.\` <@${p.id}> — \`${p.nick}\` [${p.arma || '?'}] (${p.gel})`).join('\n')
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isStringSelectMenu() && interaction.customId.startsWith('queue_arma_')) {
    ...
    return interaction.reply({ content: `✅ Arma: **${arma}**`, ephemeral: true });
  }
    ...
    if (id.startsWith('queue_normal_') || id.startsWith('queue_infinito_')) {
      ...
      await interaction.reply({
        embeds: [new EmbedBuilder().setColor(0xFFAA00)
          .setTitle(`⚔️ Entrar na Fila ${mode} — ${gelType}`)
          .setDescription(`**Valor:** ${formatVal(state.value)}\n\nDigite seu **nick no Free Fire** no chat:`)],
        ephemeral: true
      });
      ...
      collector.on('collect', async (msg) => {
        ...
        if (state.players.length >= cfg.maxPlayers)
          return interaction.editReply({ content: '❌ Fila cheia!', embeds: [], components: [] });

        state.players.push({ id: interaction.user.id, name: interaction.user.username, nick, gel: gelType, arma: null });
        try { await interaction.message.edit({ embeds: [buildQueueEmbed(mode, state.value)], components: buildQueueButtons(mode) }); } catch {}
        await interaction.editReply({ content: `✅ Na fila **${mode}** como \`${nick}\`!`, embeds: [], components: [] });
        ...
        if (state.players.length >= cfg.maxPlayers) {
          ...
          const filaEmbed = new EmbedBuilder()
            ...
            .setDescription(
              ...
              `**Jogadores:**\n` +
              players.map((p, i) => `\`${i + 1}.\` <@${p.id}> — \`${p.nick}\` | ${p.gel} | ${p.arma || 'Arma não escolhida'}`).join('\n') +
      ...
      collector.on('end', (c) => {
        if (c.size === 0) interaction.editReply({ content: '⏰ Tempo esgotado.', embeds: [], components: [] }).catch(() => {});
      });
      return;
    }
