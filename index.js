...
async function criarCanalFila(interaction, mode, state) {
  ...
  // Deleta a mensagem pública da fila
...
client.on('messageCreate', async (message) => {
  ...
  // !coin @player 10 — adm adiciona vitórias (1 a 100)
  if (cmd === 'coin') {
    if (!isAdmin) return message.reply('❌ Sem permissão.');
    const target = message.mentions.members.first();
    const amount = parseInt(args[1]);
    if (!target) return message.reply('❌ Mencione um jogador. Ex: `!coin @player 10`');
    if (isNaN(amount) || amount < 1 || amount > 100) return message.reply('❌ Valor inválido. Use de 1 a 100.');
    ...
    const embed = new EmbedBuilder()
      .setColor(0xFFAA00)
      .setTitle('🏆 ORG TIGRE')
      .setDescription(`✅ **${amount}** vitória(s) adicionada(s) para <@${target.id}>!`)
      .addFields(
  ...
  // !perdeu @player 50 — adm adiciona derrotas (1 a 2000)
  if (cmd === 'perdeu') {
    if (!isAdmin) return message.reply('❌ Sem permissão.');
    const target = message.mentions.members.first();
    const amount = parseInt(args[1]);
    if (!target) return message.reply('❌ Mencione um jogador. Ex: `!perdeu @player 50`');
    if (isNaN(amount) || amount < 1 || amount > 2000) return message.reply('❌ Valor inválido. Use de 1 a 2000.');
    const p = getPlayer(target.id);
    p.perdas += amount;
    saveConfig();
    const embed = new EmbedBuilder()
      .setColor(0xE74C3C)
      .setTitle('📊 ORG TIGRE')
      .setDescription(`📝 **${amount}** derrota(s) registrada(s) para <@${target.id}>!`)
      .addFields(
        { name: '🏆 Vitórias', value: `${p.vitorias}`, inline: true },
        { name: '✅ Ganhas', value: `${p.ganhas}`, inline: true },
        { name: '❌ Perdidas', value: `${p.perdas}`, inline: true },
  ...
  // !stats — jogador ver suas próprias stats
  if (cmd === 'stats') {
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.isButton()) {
    ...
    if (id.startsWith('queue_normal_') || id.startsWith('queue_infinito_')) {
      ...
      if (state.players.length >= cfg.maxPlayers) {
        // Deleta a mensagem da fila pública quando encher
        try { await interaction.message.delete(); } catch {}
